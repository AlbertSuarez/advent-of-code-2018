...
_INPUT_FILE = 'data/day_4_input.txt'
...
def solve():
    ...
    for log in logs:
        ...
        log_dict[date] = movement

    # noinspection PyTypeChecker
    ordered_dict = OrderedDict(sorted(log_dict.items()))

    guard_id = None
    start_time = None
    log_dict = {}
    for date, movement in ordered_dict.items():
        if movement.startswith('Guard'):
            guard_id = movement.split(' ')[1].replace('#', '')
            if guard_id not in log_dict:
                log_dict[guard_id] = dict(duration=0, minutes={})
        elif movement.startswith('falls'):
            start_time = date
        else:
            end_time = date
            duration = (end_time - start_time).total_seconds() / 60.0
            log_dict[guard_id]['duration'] += duration
            for i in range(0, int(duration)):
                minute_date = (start_time + timedelta(minutes=i)).minute
                if minute_date in log_dict[guard_id]['minutes']:
                    log_dict[guard_id]['minutes'][minute_date] += 1
                else:
                    log_dict[guard_id]['minutes'][minute_date] = 1
    ...
    for key, value in log_dict.items():
        if value['duration'] > maximum_duration:
            maximum_duration = value['duration']
            maximum_guard = key

    log_minutes = log_dict[maximum_guard]['minutes']
    sort_minutes = [(k, log_minutes[k]) for k in sorted(log_minutes, key=log_minutes.get, reverse=True)]
    for key, value in sort_minutes:
        print('Part 1: Guard * Maximum minute: {}'.format(int(maximum_guard) * int(key)))
        break
    ...
    maximum_guard = None
    for key, value in log_dict.items():
        log_minutes = value['minutes']
        sort_minutes = [(k, log_minutes[k]) for k in sorted(log_minutes, key=log_minutes.get, reverse=True)]
        if sort_minutes:
            if sort_minutes[0][1] > maximum_duration:
                maximum_duration = sort_minutes[0][1]
                maximum_minute = sort_minutes[0][0]
                maximum_guard = key

    print('Part 2: Any Guard * Maximum minute: {}'.format(int(maximum_guard) * int(maximum_minute)))

...
_INPUT_FILE = 'data/day_1_input.txt'
...
def solve_part_1():
    ...
    For example, if the device displays frequency changes of +1, -2, +3, +1, then starting from a frequency of
    zero, the following changes would occur:
    ...
    print('Part 1: Frequency = {}'.format(frequency))
...
def solve_part_2():
    ...
    In this example, the first frequency reached twice is 2. Note that your device might need to repeat its list
    of frequency changes many times before a duplicate frequency is found, and that duplicates might be found while
    ...
    while True:
        for number in numbers:
            if number:
                frequency += int(number)
                if frequency not in first_reach:
                    first_reach[int(frequency)] = True
                else:
                    print('Part 2: First reach = {}'.format(int(frequency)))
                    return
